classdef bma_tools
    ...
    methods (Static)
        function motion_vectors = hbma(anchor, target, tiers, block_size, window_size)
            % Get all motion vectors with HBMA hex search
            motion_vectors = hbma(anchor, target, tiers, block_size, window_size);
        end
        
        function motion_vectors = hex_search(anchor, target, block_size, window_size)
            ...
            motion_vectors = zeros(rows/block_size, cols/block_size, 2);
             for row = 1:block_size:rows
                for col = 1:block_size:cols
                    ...
                    [drow, dcol] = hexbs(row, col, anchor_block, target, window_size);
