function mvs = hbma(anchor, target, tier, block_size, window_size)
    % IF:
    ...
    if tier == 1 % Lowest recursion depth
        for row = 1:block_size:rows
            for col = 1:block_size:cols
                ...
                [drow, dcol] = hexbs(row, col, anchor_block, target, window_size(tier));
        ...
        prev_mvs = hbma(new_anchor, new_target, tier-1, block_size, window_size);
        ...
        for row = 1:block_size:rows
            for col = 1:block_size:cols
                ...
                anchor_block = anchor(row:row+block_size-1, col:col+block_size-1);
                [drow, dcol] = hexbs(row+prow*2, col+pcol*2, anchor_block, target, window_size(tier));
